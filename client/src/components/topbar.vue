<template>
    <nav class="navbar navbar-expand-md navbar-dark zd-bg-blue">
        <div class="d-flex zd-center mx-auto">
            ...
            <router-link class="btn ml-auto btn-outline-light zd-use-font" to="/login" v-if="this.$route.path !== '/login' && !$root.loggedIn">Log in</router-link>
            <button v-on:click="logout" class="btn ml-auto btn-outline-light zd-use-font" v-if="$root.loggedIn">Log out</button>
        </div>
    </nav>
</template>
